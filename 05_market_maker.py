...
Task
----
Basic market-making strategy on BTC-USD:
- Quote around mid with a defined half-spread (bps).
- Simulate passive fills using the trade feed (market_trades).
- Position-based skewing (adjust quotes if long/short).
- Enforce risk limits: max notional exposure = $1,000,000, max loss = $100,000.
- Continuously display: executed fills, position, avg entry, exposure, realized/unrealized/total P&L.
"""
...
# Quoting params (half-spread is small; clamping to top-of-book dominates fills)
QUOTE_SIZE_BTC = 0.10           # size per side
BASE_HALF_SPREAD_BPS = 0.1      # half-spread in bps (0.1 = 0.001%)
SKEW_BPS_AT_MAX = 8.0           # extra bps shift at "max inventory" scale
SKEW_INVENTORY_SCALE_BTC = 10.0 # inventory magnitude where full skew applies
...
class TradeBuffer:
    ...
    def add_from_ws(self, events: list) -> None:
        ...
        # Trim from the front if too long and adjust cursor accordingly
    ...
    def drain_new(self) -> List[Trade]:
        """Return trades added since last call and advance the cursor."""
...
class MarketMaker:
    ...
    def compute_quotes(self, mid: float, position_btc: float) -> Optional[Quotes]:
        ...
        # Long inventory (pos_ratio > 0): push quotes down
        bid = mid * (1.0 - half - max(0.0, skew))
        ask = mid * (1.0 + half - min(0.0, skew))
        if bid >= ask:
            # Safety: preserve a positive width
    ...
    def simulate_fills(self, prints: List[Trade], q: Quotes) -> List[Tuple[float, float]]:
        """
        Passive fill model per side (fixed the earlier VWAP mixing bug):
          - Track bid fills and ask fills independently.
          - Return up to two fills: [(+qty_btc, vwap_bid), (-qty_btc, vwap_ask)].
        """
        buy_filled = 0.0   # BTC bought at bid
        buy_cost = 0.0     # sum(qty * bid_px)
        sell_filled = 0.0  # BTC sold at ask
        sell_proceeds = 0.0  # sum(qty * ask_px)
...
def main() -> None:
    ...
    try:
        print("Market-making simulation… Ctrl+C to stop.")
        last_print = 0.0
        while True:
            time.sleep(0.1)
            ...
            if allowed:
                ...
                if q:
                    ...
                    for qty, px in mm.simulate_fills(new_prints, q):
